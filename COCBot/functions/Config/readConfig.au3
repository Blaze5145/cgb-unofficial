...
Func readConfig() ;Reads config and sets it to the variables
	If FileExists($config) Then
		...
		$deployBSettings = IniRead($config, "attack", "deployB", "1")
		$deployTHSettings = IniRead($config, "attack", "deployTH", "0")
		$deployASettings = IniRead($config, "attack", "deployA", "3")
		...
		$BalloonsComp = IniRead($config, "troop", "balloon", "0")
