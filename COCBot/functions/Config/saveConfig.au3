...
Func saveConfig() ;Saves the controls settings to the config
	...
	IniWrite($config, "attack", "deployB", _GUICtrlComboBox_GetCurSel($cmbDeployB))
	IniWrite($config, "attack", "deployTH", _GUICtrlComboBox_GetCurSel($cmbDeployTH))
	IniWrite($config, "attack", "deployA", _GUICtrlComboBox_GetCurSel($cmbDeployA))
