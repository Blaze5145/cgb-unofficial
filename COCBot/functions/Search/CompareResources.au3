...
Func CompareResources() ;Compares resources and returns true if conditions meet, otherwise returns false
	...
	If $iChkSearchReduction = 1 Then
		If $SearchCount <> 0 And Mod($SearchCount, $ReduceCount) = 0 Then
			...
			SetLog("Aim: [G]:" & StringFormat("%7s", $AimGold) & " [E]:" & StringFormat("%7s", $AimElixir) & " [D]:" & StringFormat("%5s", $AimDark) & " [T]:" & StringFormat("%2s", $AimTrophy) & $AimTHtext, $COLOR_GREEN, "Lucida Console", 7.5)
		EndIf
	EndIf

	Local $G = (Number($searchGold) >= Number($AimGold)), $E = (Number($searchElixir) >= Number($AimElixir)), $D = (Number($searchDark) >= Number($AimDark)), $T = (Number($searchTrophy) >= Number($AimTrophy))
	Local $THLO = -1
	$THL = -1
	For $i = 0 To 4
