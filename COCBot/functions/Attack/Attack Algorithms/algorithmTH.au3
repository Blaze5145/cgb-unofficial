Global $checkKPower = True,$checkQPower = True
$King = $Barb = $Arch = -1
...
;						Setlog("LL Bottom deployment $THi="&$THi)
						For $num=0 to $numperspot
							  ...
							  Local $BottomDeployCount=Number(getNormal(40 + (72 * $THtroop), 565))
							  		...
							  		Click(430,668,1,50) ; **fixes number of troops for bottom TH left side**
							  _CaptureRegion()
						...
						For $num=0 to $numperspot
							  ...
							  $BottomDeployCount=Number(getNormal(40 + (72 * $THtroop), 565))
									;Click(531,538,5,50) ;not sure deploy but highest chance of hit. Static 1 spot only
							  		Click(430,668,1,50) ; **fixes number of troops for bottom TH right side** ; Line 290
							  #cs
							  _CaptureRegion()
							  If $BottomDeployCount=Number(getNormal(40 + (72 * $THtroop), 565)) Then
...
Func AttackTHgbarch()
	...
	_CaptureRegion()
	 If _ColorCheck(_GetPixelColor(746,498), Hex(0xc8cac7, 6), 20)=True Then
		 SetLog("You get a star - Good for you!")
		 SetLog("Only used archers. Drop some giants & barbs to keep ratio even.")
		 AttackTHGrid($eGiant,1,3,1000,1,2,0) ;drop 3 giant
		 AttackTHGrid($eBarbarian,3,7,2000,1,5,0) ; drop 21 barbs
		 _Sleep(1000)
	...
	_CaptureRegion()
	 If _ColorCheck(_GetPixelColor(746,498), Hex(0xc8cac7, 6), 20)=True Then
		 SetLog("Only used archers. Drop some giants & barbs to keep ratio even.")
		 AttackTHGrid($eGiant,1,5,1500,2,2,0) ;drop 5 giants
		 AttackTHGrid($eBarbarian,3,7,1500,1,5,0) ; drop 21 barbs
		 AttackTHGrid($eBarbarian,3,7,1000,1,5,0) ; drop 21 barbs
		 SetLog("You get a star - Good for you!")
		 _Sleep(1000)
	 Return ;exit if you get a star
	 EndIf
	...
	AttackTHGrid($eBarbarian,4,6,1000,1,5,0) ; deploys up to 24 barbarians
	AttackTHGrid($eBarbarian,4,6,1500,1,5,1) ; deploys up to 24 barbarians & drops Heroes + CC
	...
	_CaptureRegion()
	 ...
	 Return ;exit if you get a star
	 EndIf
	SetLog("If you have troops remaining, be sure to use Barracks Mode!")
	_Sleep(1000)
...
;		 SetLog("Attacking TH with 1st wave of BAM")
		 AttackTHGrid($eBarbarian,5,1,5000,1,5,0) ; deploys 5 barbarians
		 AttackTHGrid($eArcher,5,1,5000,1,4,0) ; deploys 5 archers
		 AttackTHGrid($eMinion,5,1,5000,1,4,0) ; deploys 5 minions
		 ;---2nd wave 20 secs
; 		 SetLog("Attacking TH with 2nd wave of BAM")
		 AttackTHGrid($eBarbarian,5,1,5000,2,5,0) ; deploys 5 barbarians
		 AttackTHGrid($eArcher,5,1,7500,2,4,0) ; deploys 5 archers
		 AttackTHGrid($eMinion,5,1,7500,2,4,0) ; deploys 5 minions
		 ;---3nd wave 10 secs
; 		 SetLog("Attacking TH with 3rd wave of BAM COMBO")
		 AttackTHGrid($eBarbarian,5,1,10,3,5,0) ; deploys 5 barbarians
		 AttackTHGrid($eMinion,5,1,10,3,4,0) ; deploys 5 minions
		 AttackTHGrid($eArcher,5,1,1000,3,4,0) ; deploys 5 archers
...
;		 SetLog("Attacking TH with 4th wave of Giant BAM Troops.. Xtreme Deployment")
		 ...
		 AttackTHGrid($eWallBreaker,2,2,10,1,1,0) ; deploys 4 wallbreakers
		 AttackTHGrid($eArcher,5,5,7000,4,4,0) ;releases 25 archers
		 AttackTHGrid($eBarbarian,5,5,7000,4,5,0) ;releases 25 barbs
		 AttackTHGrid($eMinion,5,2,5000,4,4,1) ;releases 10 minions and Heroes
...
;		 SpellTHGrid($eRSpell)
 		 ...
 		 AttackTHGrid($eHog,5,1,100,2,4,0) ;releases 5 Hogs
		 AttackTHGrid($eArcher,5,5,7000,4,4,0) ;releases 25 archers
		 AttackTHGrid($eBarbarian,5,5,7000,4,5,0) ;releases 25 barbs
		 AttackTHGrid($eMinion,5,2,5000,4,4,0) ;releases 10 minions
		 AttackTHGrid($eWizard,3,2,10000,1,1,1) ;releases 6 wizards and releases hero
...
;		 SetLog("Attacking TH with 1st wave of BAM COMBO")
		 AttackTHGrid($eBarbarian,5,1,10,1,5,0) ; deploys 5 barbarians
		 AttackTHGrid($eArcher,5,1,10,1,4,0) ; deploys 5 archers
		 AttackTHGrid($eMinion,5,1,10000,1,4,0) ; deploys 5 minions
...
;		 SetLog("Attacking TH with 2nd wave of BAM COMBO")
		 AttackTHGrid($eBarbarian,5,1,10,2,5,0) ; deploys 5 barbarians
		 AttackTHGrid($eMinion,5,1,10,2,4,0) ; deploys 5 minions
		 AttackTHGrid($eArcher,5,1,10000,2,4,0) ; deploys 5 archers
...
;		 SetLog("Attacking TH with 3rd wave of BAM COMBO")
		 AttackTHGrid($eBarbarian,5,1,10,3,5,0) ; deploys 5 barbarians
		 AttackTHGrid($eMinion,5,1,10,3,4,0) ; deploys 5 minions
		 AttackTHGrid($eArcher,5,1,12000,3,4,0) ; deploys 5 archers
		 If _ColorCheck(_GetPixelColor(284, 28), Hex(0x41B1CD, 6), 20) Or _ColorCheck(_GetPixelColor(426,562), Hex(0x60AC10, 6), 20)  Then Return ;~~ If return button found or mainscreen,proceed
...
; 		 SpellTHGrid($eHSpell)
		 ...
		 AttackTHGrid($eArcher,5,5,200,4,4,0) ;releases 25 archers
		 AttackTHGrid($eBarbarian,5,5,200,4,5,0) ;releases 25 barbs
		 AttackTHGrid($eMinion,5,2,200,4,4,0) ; deploys 5 minions
 		 AttackTHGrid($eGiant,5,1,100,2,4,0) ;releases 5 giants
 		 AttackTHGrid($eHog,5,1,100,2,4,0) ;releases 5 Hogs
;		 AttackTHGrid($eGolem,5,1,100,3,4,0) ;releases 5 Golems
;		 AttackTHGrid($ePekka,5,1,100,4,4,0) ;releases 5 Pekkas
 		 AttackTHGrid($eBarbarian,5,5,200,5,5,0) ;releases 25 barbs
		 AttackTHGrid($eWizard,3,2,10000,1,1,1) ;releases 6 wizards and releases hero
...
;		 AttackTHGrid($eDragon,3,1,500,1,1,0) ;releases 3 dragons
;		 AttackTHGrid($eBalloon,5,2,1000,1,1,0) ;releases 10 balloons
;		 AttackTHGrid($eMinion,5,2,7000,1,1,1) ;releases 10 minions and releases hero
;		 If _ColorCheck(_GetPixelColor(284, 28), Hex(0x41B1CD, 6), 20) Or _ColorCheck(_GetPixelColor(426,562), Hex(0x60AC10, 6), 20)  Then Return ;~~ If return button found or mainscreen,proceed
